...
The question we wanted to address was, How similar/different are floral scent profiles between wind and insect pollinated species? One classic method is  assesing this with multivariate scent data is non-multidimensional scaling (nMDS). This method reprsents the pariwise disimilarities in low dimensional space, similar to a Principle Component Analysis (PCA).

However our data contains member of different species, which may share similarity simply because they are closely related. Therefore it is important to asses whether species resemble each other  needed to test whether species resmelbled each other simply due to phylogenetic histoyr. 

The following is a slight modificaiton from the published supplementary material found in this [github repo](https://github.com/Jesusthebotanist/Thalictrum-Floral-Scent). The nMDS analysis is written in Matlab the programming language taught in BIOl419 while the rest is written in R (I switched half way through R is free). I walk through both scripts below:

## nMDS in Matlab

The data is floral scent analyzed by gas chromatography-mass spectrometry from 11 species of Thalictrum and electroantennogram (EAG) responses from  buff-tailed bumblebee antenna exposed to floral scent extract from each *Thalictrum* species. This script does two things,

1) Calculates Blomberg's K for the Floral scent data and the EAG

2) Performed an ancestral state reconstrion of bumble bee EAG response/generates Figure 4 from the manuscript
...
This is traditionally performed on single variable, however biologsit are often intrested if the entire set of variable are similar. Because our GCMS data is multivariate (~50 compounds), we can calculate Blombergs K for each individaul compound as well as the entire datset as a whole. 
